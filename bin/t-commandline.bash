...
az_auth() {
	printf "\nPlease type your Azure UserName : "
	read AZ_UNAME
	printf "\nPlease type your Azure Password : "
    read AZ_PWD
	az login -u $AZ_UNAME -p $AZ_PWD > .azure.accounts
}
t-deploy-lab() {
	...
	then
		...
		clientID=b580ab71-eb18-40ed-994d-5a4870c54259
		clientSecret=-
	...
	elif [[ $(awk -F= -v key="cloud_provider" '$1==key {print $2}' /terraTrain/config  | tr -d '"' | cut -d' ' -f1 | tr -d "\n") == "aws" ]]
	then
		cd /terraTrain/modules/aws/
		terraform init -input=false
...
t-destroy-lab() {
	...
	then
		...
		printf "\nWrong stuff dude. What da hell you did??\n"
	fi
	################
	echo " " > launchpad.yaml
}
...
### t show access-key-linux|access-key-windows
t-show-access_key_linux() {
	cat /terraTrain/key-pair
	printf "\nYou also can find the private key here:\n/terraTrain/key-pair"
	printf "\nUsage: ssh -i /terraTrain/key-pair ....\n"
}
t-show-access_key_windows() {
	printf "\nYou can use the following information for accessing windows Nodes\n"
	printf "\nUsername: Administrator\n"
	printf "\nPassword: $(cat /terraTrain/terraform.tfstate 2>/dev/null | jq '.resources[] | select(.name=="mke_password") | .instances[] | .attributes.result' 2>/dev/null)\n"
}
...
show|sh) case "$2" in
			...
			a|al|all) t-show-all
					exit ;;
			akl|access-key-linux) t-show-access_key_linux
					exit;;
			akw|access-key-windows) t-show-access_key_windows
					exit;;
